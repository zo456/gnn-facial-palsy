...
def split_data_clean(data_dir, split):
    ...
    for subdir in subdirs:
        ...
        set_, set0 = train_test_split(list(img_dict.keys()), test_size=0.25, shuffle = True, random_state=3546)
        set_, set1 = train_test_split(set_, test_size=0.33, shuffle=True, random_state=45)
        set2, set3 = train_test_split(set_, test_size=0.5, shuffle=True, random_state=45)
        ...
        if split == 2:
            ...
            images_test = images1
        if split == 3:
            images_train = images3 + images0# + images0
            images_val = images1
            images_test = images2

    return images_train, images_val, images_test

def split_data_pose(data_dir, split):
    subdirs = os.listdir(data_dir)
    temp = []
    for subdir in subdirs:
        if subdir.endswith('landmarks'):
    ...
    for subdir in subdirs:
        ...
        for item in img_list:
            ...
            if patient_id[0] not in img_dict.keys():
                img_dict[patient_id[0]] = []
            img_dict[patient_id[0]].append(data_dir + os.sep + subdir + os.sep + item)
        set_, set0 = train_test_split(list(img_dict.keys()), test_size=0.33, shuffle = True, random_state=3546)
        set1, set2 = train_test_split(set_, test_size=0.5, shuffle=True, random_state=45)
...
def split_data_mixed(data_dir, split):
    subdirs = os.listdir(data_dir)
    temp = []
    for subdir in subdirs:
        if subdir.endswith('landmarks'):
...
class GraphDataset(InMemoryDataset):
    def __init__(self, root, split=0, mode='train', type='clean', transform=None, pre_transform=None, pre_filter=None, log=False):
        super().__init__(root, transform, pre_transform, pre_filter, log)
        self.type = type
        if self.type == 'clean':
            self.train_list, self.val_list, self.test_list = split_data_clean(root, split)
        elif self.type == 'pose':
            self.train_list, self.val_list, self.test_list = split_data_pose(root, split)
        else:
            self.train_list, self.val_list, self.test_list = split_data_mixed(root, split)
