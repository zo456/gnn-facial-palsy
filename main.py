...
parser.add_argument('--type', default='mixed', type=str, help='Experiment dataset type to choose depending on dataset cleanness', choices=['clean', 'mixed', 'pose'])
...
def main():
    if args.type == 'clean':
        num_split = 4
        data_dir = '../graph (25th copy)/tnf/'
    elif args.type == 'mixed':
        num_split = 5
        data_dir = '../graph (25th copy)/tnf-mixed/'
    else:
        num_split = 3
        data_dir = '../graph (25th copy)/tnf-bad/'
    for i in range(num_split):
        with LoggingPrinter(f'{args.save_model}_split_{i}.txt'):
            train_loader, val_loader, test_loader = graph_load.LoadGraphData(data_dir, args.batch_size, args.batch_size, args.type ,i)
            ...
            #scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.95, threshold=1e-2, patience=100, cooldown=50)
            check = 0
            track = 0
            train_track = 0
            old_lr = 0
                ...
                if train_track < args.tolerance:
                    if track < args.tolerance:
                        ...
                        if epoch % 200 == 0:
                            ...
                            torch.save(model.state_dict(), f'checkpoint_{args.save_model}_split_{i}.pt')
